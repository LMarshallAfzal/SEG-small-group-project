...
class chessViewTestCase(TestCase):
    """Tests of the chess view."""
    ...
    def setUp(self):
        ...
        self.url = reverse('chess')

    def test_chess_url(self):
        self.assertEqual(self.url,'/chess/')

    def test_get_chess(self):
    ...
    def test_get_chess_redirects_when_not_logged_in(self):
        redirect_url = reverse_with_next('log_in', self.url)
        response = self.client.get(self.url)
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)

    def test_chess_contains_posts_by_self_and_followees(self):
        self.client.login(username=self.user.username, password='Password123')
